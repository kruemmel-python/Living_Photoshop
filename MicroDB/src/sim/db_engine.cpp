...
bool db_run_ingest(DbWorld &world, const DbIngestConfig &cfg, std::string &error) {
    ...
    const bool dump_frames = cfg.dump_every > 0;
    std::filesystem::path dump_dir = cfg.dump_dir.empty() ? "frames" : cfg.dump_dir;
    if (dump_frames) {
        std::error_code ec;
    ...
    for (int step = 0; step < cfg.steps; ++step) {
        ...
        if (dump_frames && (step % cfg.dump_every == 0)) {
            std::ostringstream name;
            name << cfg.dump_prefix << "_" << std::setw(5) << std::setfill('0') << step << ".ppm";
            std::filesystem::path out_path = dump_dir / name.str();
            std::string dump_error;
            db_save_cluster_ppm(out_path.string(), world, cfg.dump_scale, dump_error);
        }
    }
