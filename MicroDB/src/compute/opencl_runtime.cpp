...
struct OpenCLRuntime::Impl {
    ...
    cl_kernel diffuse_kernel_3d = nullptr;
    ...
    void release_all() {
        ...
        if (diffuse_kernel_3d) {
            OCL_CALL(clReleaseKernel)(diffuse_kernel_3d);
            diffuse_kernel_3d = nullptr;
        }
        if (program) {
...
bool OpenCLRuntime::build_kernels(std::string &error) {
    ...
    impl->diffuse_kernel_3d = OCL_CALL(clCreateKernel)(impl->program, "diffuse_and_evaporate_3d", &err);
    if (!impl->diffuse_kernel_3d || err != CL_SUCCESS) {
        impl->diffuse_kernel_3d = nullptr;
    }
    return true;
}
...
bool OpenCLRuntime::init_fields(const GridField &phero_food,
    ...
    if (phero_food.width != molecules.width || phero_food.height != molecules.height || phero_food.depth != molecules.depth ||
        phero_food.width != phero_danger.width || phero_food.height != phero_danger.height || phero_food.depth != phero_danger.depth) {
    ...
    impl->depth = std::max(1, phero_food.depth);
    size_t bytes = static_cast<size_t>(impl->width) * impl->height * impl->depth * sizeof(float);
    cl_int err = CL_SUCCESS;
...
bool OpenCLRuntime::upload_fields(const GridField &phero_food,
                                  ...
                                  const GridField &molecules,
                                  std::string &error) {
    if (phero_food.width != impl->width || phero_food.height != impl->height || phero_food.depth != impl->depth) {
...
bool OpenCLRuntime::step_diffuse(const FieldParams &pheromone_params,
    ...
    if (impl->depth > 1 && !impl->diffuse_kernel_3d) {
        error = "OpenCL 3D kernel not available";
        return false;
    }
    size_t global2[2] = {static_cast<size_t>(impl->width), static_cast<size_t>(impl->height)};
    size_t global3[3] = {static_cast<size_t>(impl->width), static_cast<size_t>(impl->height), static_cast<size_t>(impl->depth)};
    auto run_kernel = [&](cl_mem in_buf, cl_mem out_buf, const FieldParams &params) -> bool {
        cl_int err = CL_SUCCESS;
        if (impl->depth > 1 && impl->diffuse_kernel_3d) {
            err |= OCL_CALL(clSetKernelArg)(impl->diffuse_kernel_3d, 0, sizeof(cl_mem), &in_buf);
            err |= OCL_CALL(clSetKernelArg)(impl->diffuse_kernel_3d, 1, sizeof(cl_mem), &out_buf);
            err |= OCL_CALL(clSetKernelArg)(impl->diffuse_kernel_3d, 2, sizeof(int), &impl->width);
            err |= OCL_CALL(clSetKernelArg)(impl->diffuse_kernel_3d, 3, sizeof(int), &impl->height);
            err |= OCL_CALL(clSetKernelArg)(impl->diffuse_kernel_3d, 4, sizeof(int), &impl->depth);
            err |= OCL_CALL(clSetKernelArg)(impl->diffuse_kernel_3d, 5, sizeof(float), &params.diffusion);
            err |= OCL_CALL(clSetKernelArg)(impl->diffuse_kernel_3d, 6, sizeof(float), &params.evaporation);
            if (err != CL_SUCCESS) {
                error = std::string("clSetKernelArg failed: ") + cl_err_to_string(err);
                return false;
            }
            err = OCL_CALL(clEnqueueNDRangeKernel)(impl->queue, impl->diffuse_kernel_3d, 3, nullptr, global3, nullptr, 0, nullptr, nullptr);
        } else {
            ...
            err = OCL_CALL(clEnqueueNDRangeKernel)(impl->queue, impl->diffuse_kernel, 2, nullptr, global2, nullptr, 0, nullptr, nullptr);
        }
        if (err != CL_SUCCESS) {
