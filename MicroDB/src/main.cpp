...
struct CliOptions {
    ...
    int db_dump_scale = 4;
    int db_dump_every = 0;
    std::string db_dump_dir = "frames";
    std::string db_dump_prefix = "frame";
...
void print_help() {
    std::cout << "micro_swarm Optionen:\n"
              ...
              << "  --db-dump-every N  PPM-Frame alle N Schritte (0=aus)\n"
              << "  --db-dump-dir PATH  Ausgabeordner fuer PPM-Frames\n"
              << "  --db-dump-prefix N  Dateiprefix fuer PPM-Frames\n"
              ...
              << "  --dna-import PATH                DNA CSV importieren\n"
              << "  --dna-export PATH                DNA CSV exportieren\n"
              << "  --dna-style NAME                 Style-Tag fuer DNA-Archiv\n"
              << "  --help           Hilfe anzeigen\n";
}
...
void clamp_genome_local(Genome &g) {
...
bool export_dna_csv(const std::string &path,
    ...
    out << "pool,species,fitness,sense_gain,pheromone_gain,exploration_bias,edge_seek,soften_bias,mycel_drive,blur_pref,"
           "sharpen_pref,color_shift_r,color_shift_g,color_shift_b,contrast_pulse,style\n";
    for (int s = 0; s < 4; ++s) {
...
bool import_dna_csv(const std::string &path,
                    const SimParams &params,
                    const EvoParams &evo,
                    std::array<DNAMemory, 4> &dna_species,
                    DNAMemory &dna_global,
                    const std::string &style_default) {
    ...
    while (std::getline(in, line)) {
        ...
        try {
            ...
            clamp_genome_local(g);
            const std::string style = style_str.empty() ? style_default : style_str;
            if (pool == "global") {
                dna_global.add(params, g, fitness, evo, params.dna_global_capacity, style);
            } else {
                if (species >= 0 && species < 4) {
                    dna_species[species].add(params, g, fitness, evo, params.dna_capacity, style);
                }
            }
        } catch (...) {
            continue;
...
bool parse_cli(int argc, char **argv, CliOptions &opts) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--mode") {
        ...
        } else if (arg == "--db-dump-every") {
            if (!parse_int(value, opts.db_dump_every)) {
                std::cerr << "Ungueltiger Wert fuer " << arg << "\n";
                return false;
            }
        } else if (arg == "--db-dump-dir") {
            if (!parse_string(value, opts.db_dump_dir)) {
                std::cerr << "Ungueltiger Wert fuer " << arg << "\n";
                return false;
            }
        } else if (arg == "--db-dump-prefix") {
            if (!parse_string(value, opts.db_dump_prefix)) {
                std::cerr << "Ungueltiger Wert fuer " << arg << "\n";
                return false;
            }
            ...
            if (!parse_string(value, opts.dna_import)) {
                std::cerr << "Ungueltiger Wert fuer " << arg << "\n";
                return false;
            }
        } else if (arg == "--dna-export") {
            if (!parse_string(value, opts.dna_export)) {
                std::cerr << "Ungueltiger Wert fuer " << arg << "\n";
                return false;
            }
        } else if (arg == "--dna-style") {
            if (!parse_string(value, opts.dna_style)) {
                std::cerr << "Ungueltiger Wert fuer " << arg << "\n";
                return false;
            }
            ...
            if (!parse_int(value, opts.params.depth) || opts.params.depth < 1) {
                std::cerr << "Ungueltiger Wert fuer " << arg << "\n";
                return false;
            }
            opts.depth_set = true;
...
int main(int argc, char **argv) {
    ...
    if (opts.mode == "db_ingest") {
        ...
        cfg.dump_every = opts.db_dump_every;
        cfg.dump_scale = opts.db_dump_scale;
        cfg.dump_dir = opts.db_dump_dir;
        cfg.dump_prefix = opts.db_dump_prefix;
    ...
    VolumeData resources_volume;
    VolumeData pheromone_volume;
    VolumeData molecules_volume;
    std::string error;

    auto apply_dataset = [&](const std::string &path, GridData &data, VolumeData &volume, const char *label) -> bool {
        if (path.empty()) return true;
        if (params.depth > 1) {
            if (!load_volume_csv(path, volume, error)) {
                std::cerr << label << ": " << error << "\n";
                return false;
            }
            if (opts.width_set && volume.width != params.width) {
                std::cerr << "Breite aus CSV passt nicht zu --width\n";
                return false;
            }
            if (opts.height_set && volume.height != params.height) {
                std::cerr << "Hoehe aus CSV passt nicht zu --height\n";
                return false;
            }
            if (opts.depth_set && volume.depth != params.depth) {
                std::cerr << "Tiefe aus CSV passt nicht zu --depth\n";
                return false;
            }
    ...
    if (!apply_dataset(opts.resources_path, resources_data, resources_volume, "resources")) return 1;
    if (!apply_dataset(opts.pheromone_path, pheromone_data, pheromone_volume, "pheromone")) return 1;
    if (!apply_dataset(opts.molecules_path, molecules_data, molecules_volume, "molecules")) return 1;

    OpenCLStatus ocl_probe = probe_opencl();
    std::cout << "[OpenCL] " << ocl_probe.message << "\n";

    Environment env(params.width, params.height, params.depth);
    if (!resources_volume.values.empty()) {
        env.resources.data = resources_volume.values;
    } else if (!resources_data.values.empty()) {
        env.resources.data = resources_data.values;
    } else {
        env.seed_resources(rng);
    }

    GridField phero_food(params.width, params.height, params.depth, 0.0f);
    GridField phero_danger(params.width, params.height, params.depth, 0.0f);
    GridField molecules(params.width, params.height, params.depth, 0.0f);
    MycelNetwork mycel(params.width, params.height, params.depth);
    if (!pheromone_volume.values.empty()) {
        phero_food.data = pheromone_volume.values;
    } else if (!pheromone_data.values.empty()) {
        phero_food.data = pheromone_data.values;
    }
    if (!molecules_volume.values.empty()) {
        molecules.data = molecules_volume.values;
    } else if (!molecules_data.values.empty()) {
    ...
    auto maybe_add_global = [&](const Genome &genome, float fitness) {
        ...
        if (dna_global.entries.size() < static_cast<size_t>(params.dna_global_capacity)) {
            dna_global.add(params, genome, fitness, evo, params.dna_global_capacity, opts.dna_style);
            return;
        }
    ...
    if (!opts.dna_import.empty()) {
        if (!import_dna_csv(opts.dna_import, params, evo, dna_species, dna_global, opts.dna_style)) {
            std::cerr << "[warn] DNA Import fehlgeschlagen: " << opts.dna_import << "\n";
        }
    }
    ...
    for (int i = 0; i < params.agent_count; ++i) {
        ...
        agent.z = (params.depth > 1) ? static_cast<float>(rng.uniform_int(0, params.depth - 1)) : 0.0f;
        agent.heading = rng.uniform(0.0f, 6.283185307f);
        agent.pitch = (params.depth > 1) ? rng.uniform(-1.0f, 1.0f) : 0.0f;
    ...
    auto dump_fields = [&](int step) -> bool {
        ...
        auto dump_one = [&](const std::string &suffix, const GridField &field) -> bool {
            ...
            if (field.depth > 1) {
                if (!save_volume_csv(path.string(), field.width, field.height, field.depth, field.data, error)) {
                    std::cerr << error << "\n";
                    return false;
                }
                return true;
        ...
        for (auto &agent : agents) {
            ...
            if (opts.evo_enable) {
                if (agent.energy > opts.evo_min_energy_to_store) {
                    dna_species[agent.species].add(params, agent.genome, agent.fitness_value, evo, params.dna_capacity, opts.dna_style);
                    maybe_add_global(agent.genome, agent.fitness_value);
                    agent.energy *= 0.6f;
                }
            } else {
                if (agent.energy > 1.2f) {
                    dna_species[agent.species].add(params, agent.genome, agent.energy, evo, params.dna_capacity, opts.dna_style);
                    agent.energy *= 0.6f;
                }
            }
        }
    ...
    if (opts.dump_every > 0) {
        std::cout << "Speichere finalen Zustand (Step " << params.steps << ")...\n";
        if (!dump_fields(params.steps)) {
            return 1;
        }
    }
    ...
    if (!opts.dna_export.empty()) {
        if (!export_dna_csv(opts.dna_export, dna_species, dna_global)) {
            std::cerr << "[warn] DNA Export fehlgeschlagen: " << opts.dna_export << "\n";
        }
    }
