...
void clamp_genome(Genome &g) {
    ...
    g.edge_seek = std::min(1.0f, std::max(-1.0f, g.edge_seek));
    g.soften_bias = std::min(1.0f, std::max(0.0f, g.soften_bias));
    g.mycel_drive = std::min(2.0f, std::max(0.0f, g.mycel_drive));
    g.blur_pref = std::min(1.0f, std::max(0.0f, g.blur_pref));
    g.sharpen_pref = std::min(1.0f, std::max(0.0f, g.sharpen_pref));
    g.color_shift_r = std::min(0.5f, std::max(-0.5f, g.color_shift_r));
    g.color_shift_g = std::min(0.5f, std::max(-0.5f, g.color_shift_g));
    g.color_shift_b = std::min(0.5f, std::max(-0.5f, g.color_shift_b));
    g.contrast_pulse = std::min(1.0f, std::max(0.0f, g.contrast_pulse));
}
...
void init_agents(MicroSwarmContext *ctx) {
    ...
    auto random_genome = [&]() -> Genome {
        ...
        g.edge_seek = ctx->rng.uniform(-0.3f, 0.3f);
        g.soften_bias = ctx->rng.uniform(0.2f, 0.8f);
        g.mycel_drive = ctx->rng.uniform(0.2f, 1.2f);
        g.blur_pref = ctx->rng.uniform(0.2f, 0.8f);
        g.sharpen_pref = ctx->rng.uniform(0.2f, 0.8f);
        g.color_shift_r = ctx->rng.uniform(-0.2f, 0.2f);
        g.color_shift_g = ctx->rng.uniform(-0.2f, 0.2f);
        g.color_shift_b = ctx->rng.uniform(-0.2f, 0.2f);
        g.contrast_pulse = ctx->rng.uniform(0.0f, 0.6f);
        return g;
    };
    auto apply_role_mutation = [&](Genome &g, const SpeciesProfile &profile) {
        ...
        if (sigma > 0.0f) {
            ...
            g.mycel_drive *= ctx->rng.uniform(1.0f - sigma, 1.0f + sigma);
        }
        if (delta > 0.0f) {
            g.exploration_bias += ctx->rng.uniform(-delta, delta);
            g.edge_seek += ctx->rng.uniform(-delta, delta);
            g.soften_bias += ctx->rng.uniform(-delta, delta);
            g.blur_pref += ctx->rng.uniform(-delta, delta);
            g.sharpen_pref += ctx->rng.uniform(-delta, delta);
            g.color_shift_r += ctx->rng.uniform(-delta, delta) * 0.5f;
            g.color_shift_g += ctx->rng.uniform(-delta, delta) * 0.5f;
            g.color_shift_b += ctx->rng.uniform(-delta, delta) * 0.5f;
            g.contrast_pulse += ctx->rng.uniform(-delta, delta);
        }
        clamp_genome(g);
    };
    ...
    for (int i = 0; i < ctx->params.agent_count; ++i) {
        ...
        agent.z = (ctx->params.depth > 1) ? static_cast<float>(ctx->rng.uniform_int(0, ctx->params.depth - 1)) : 0.0f;
        agent.heading = ctx->rng.uniform(0.0f, 6.283185307f);
        agent.pitch = (ctx->params.depth > 1) ? ctx->rng.uniform(-1.0f, 1.0f) : 0.0f;
...
void init_fields(MicroSwarmContext *ctx) {
    ctx->env = Environment(ctx->params.width, ctx->params.height, ctx->params.depth);
    ctx->env.seed_resources(ctx->rng);
    ctx->phero_food = GridField(ctx->params.width, ctx->params.height, ctx->params.depth, 0.0f);
    ctx->phero_danger = GridField(ctx->params.width, ctx->params.height, ctx->params.depth, 0.0f);
    ctx->molecules = GridField(ctx->params.width, ctx->params.height, ctx->params.depth, 0.0f);
    ctx->mycel = MycelNetwork(ctx->params.width, ctx->params.height, ctx->params.depth);
}
    ...
    for (auto &agent : ctx->agents) {
        ...
        if (ctx->evo.enabled) {
            if (agent.energy > ctx->evo_min_energy_to_store) {
                ctx->dna_species[agent.species].add(ctx->params, agent.genome, agent.fitness_value, ctx->evo,
                                                    ctx->params.dna_capacity, ctx->dna_style);
                float eps = 1e-6f;
                if (ctx->params.dna_global_capacity > 0) {
                    if (ctx->dna_global.entries.size() < static_cast<size_t>(ctx->params.dna_global_capacity) ||
                        ...
                        ctx->dna_global.add(ctx->params, agent.genome, agent.fitness_value, ctx->evo,
                                            ctx->params.dna_global_capacity, ctx->dna_style);
                    }
                }
                agent.energy *= 0.6f;
            }
            ...
            if (agent.energy > 1.2f) {
                ctx->dna_species[agent.species].add(ctx->params, agent.genome, agent.energy, ctx->evo,
                                                    ctx->params.dna_capacity, ctx->dna_style);
                agent.energy *= 0.6f;
            }
        }
...
void set_params_from_api(MicroSwarmContext *ctx, const ms_params_t &p) {
    ...
    ctx->params.depth = std::max(1, p.depth);
...
void ms_get_field_info_3d(ms_handle_t *h, ms_field_kind kind, int *w, int *hgt, int *d) {
    if (!h || !w || !hgt || !d) return;
    auto *ctx = reinterpret_cast<MicroSwarmContext *>(h);
    GridField *field = select_field(ctx, kind);
    if (!field) {
...
int ms_copy_field_out(ms_handle_t *h, ms_field_kind kind, float *dst, int dst_count) {
    ...
    GridField *field = select_field(ctx, kind);
    if (!field) return 0;
    int count = field->width * field->height * field->depth;
...
int ms_copy_field_region(ms_handle_t *h, ms_field_kind kind, int x0, int y0, int z0, int w, int hgt, int d,
                         float *dst, int dst_count) {
    if (!h || !dst) return 0;
    auto *ctx = reinterpret_cast<MicroSwarmContext *>(h);
    if (!ensure_host_fields(ctx)) return 0;
    GridField *field = select_field(ctx, kind);
    if (!field) return 0;
    if (w <= 0 || hgt <= 0 || d <= 0) return 0;
    if (x0 < 0 || y0 < 0 || z0 < 0 || x0 + w > field->width || y0 + hgt > field->height || z0 + d > field->depth) {
        return 0;
    }
    int count = w * hgt * d;
    if (dst_count < count) return 0;
    ...
    for (int z = 0; z < d; ++z) {
        for (int y = 0; y < hgt; ++y) {
            for (int x = 0; x < w; ++x) {
                dst[idx++] = field->at(x0 + x, y0 + y, z0 + z);
            }
        }
    }
    return count;
...
int ms_write_field_region(ms_handle_t *h, ms_field_kind kind, int x0, int y0, int z0, int w, int hgt, int d,
    ...
    for (int z = 0; z < d; ++z) {
        for (int y = 0; y < hgt; ++y) {
            for (int x = 0; x < w; ++x) {
                field->at(x0 + x, y0 + y, z0 + z) = src[idx++];
            }
        }
    }
    if (ctx->ocl_active) {
...
int ms_get_agents(ms_handle_t *h, ms_agent_t *out, int max_agents) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        out[i].edge_seek = a.genome.edge_seek;
        out[i].soften_bias = a.genome.soften_bias;
        out[i].mycel_drive = a.genome.mycel_drive;
        out[i].blur_pref = a.genome.blur_pref;
        out[i].sharpen_pref = a.genome.sharpen_pref;
        out[i].color_shift_r = a.genome.color_shift_r;
        out[i].color_shift_g = a.genome.color_shift_g;
        out[i].color_shift_b = a.genome.color_shift_b;
        out[i].contrast_pulse = a.genome.contrast_pulse;
    }
    return count;
}
...
void ms_set_agents(ms_handle_t *h, const ms_agent_t *agents, int count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        a.genome.edge_seek = agents[i].edge_seek;
        a.genome.soften_bias = agents[i].soften_bias;
        a.genome.mycel_drive = agents[i].mycel_drive;
        a.genome.blur_pref = agents[i].blur_pref;
        a.genome.sharpen_pref = agents[i].sharpen_pref;
        a.genome.color_shift_r = agents[i].color_shift_r;
        a.genome.color_shift_g = agents[i].color_shift_g;
        a.genome.color_shift_b = agents[i].color_shift_b;
        a.genome.contrast_pulse = agents[i].contrast_pulse;
...
void ms_spawn_agents(ms_handle_t *h, const ms_agent_t *agents, int count) {
    if (!h || !agents || count <= 0) return;
    auto *ctx = reinterpret_cast<MicroSwarmContext *>(h);
    ctx->agents.reserve(ctx->agents.size() + static_cast<size_t>(count));
    for (int i = 0; i < count; ++i) {
        Agent a;
        a.x = agents[i].x;
        a.y = agents[i].y;
...
int ms_extract_agents_outside(ms_handle_t *h, int x0, int y0, int z0, int x1, int y1, int z1,
                              ms_agent_t *out, int max_agents) {
    if (!h || !out || max_agents <= 0) return 0;
    auto *ctx = reinterpret_cast<MicroSwarmContext *>(h);
    int extracted = 0;
    auto in_bounds = [&](const Agent &a) {
        return a.x >= x0 && a.y >= y0 && a.z >= z0 && a.x < x1 && a.y < y1 && a.z < z1;
    };
    for (auto it = ctx->agents.begin(); it != ctx->agents.end() && extracted < max_agents; ) {
        if (!in_bounds(*it)) {
            ms_agent_t dst{};
            dst.x = it->x;
            dst.y = it->y;
            dst.z = it->z;
            dst.heading = it->heading;
            dst.pitch = it->pitch;
            dst.energy = it->energy;
            dst.species = it->species;
            dst.sense_gain = it->genome.sense_gain;
            dst.pheromone_gain = it->genome.pheromone_gain;
            dst.exploration_bias = it->genome.exploration_bias;
            dst.edge_seek = it->genome.edge_seek;
            dst.soften_bias = it->genome.soften_bias;
            dst.mycel_drive = it->genome.mycel_drive;
            dst.blur_pref = it->genome.blur_pref;
            dst.sharpen_pref = it->genome.sharpen_pref;
            dst.color_shift_r = it->genome.color_shift_r;
            dst.color_shift_g = it->genome.color_shift_g;
            dst.color_shift_b = it->genome.color_shift_b;
            dst.contrast_pulse = it->genome.contrast_pulse;
            out[extracted++] = dst;
            it = ctx->agents.erase(it);
        } else {
            ++it;
        }
    }
...
void ms_spawn_agent(ms_handle_t *h, const ms_agent_t *agent) {
    ...
    a.genome.edge_seek = agent->edge_seek;
    a.genome.soften_bias = agent->soften_bias;
    a.genome.mycel_drive = agent->mycel_drive;
    a.genome.blur_pref = agent->blur_pref;
    a.genome.sharpen_pref = agent->sharpen_pref;
    a.genome.color_shift_r = agent->color_shift_r;
    a.genome.color_shift_g = agent->color_shift_g;
    a.genome.color_shift_b = agent->color_shift_b;
    a.genome.contrast_pulse = agent->contrast_pulse;
    clamp_genome(a.genome);
    ctx->agents.push_back(a);
    ctx->params.agent_count = static_cast<int>(ctx->agents.size());
}
...
void ms_set_dna_style(ms_handle_t *h, const char *style) {
    if (!h) return;
    auto *ctx = reinterpret_cast<MicroSwarmContext *>(h);
    ctx->dna_style = style ? style : "";
}

int ms_get_dna_style(ms_handle_t *h, char *dst, int dst_size) {
    if (!h) return 0;
    auto *ctx = reinterpret_cast<MicroSwarmContext *>(h);
    return copy_string(dst, dst_size, ctx->dna_style);
}
...
int ms_export_dna_csv(ms_handle_t *h, const char *path) {
    ...
    out << "pool,species,fitness,sense_gain,pheromone_gain,exploration_bias,edge_seek,soften_bias,mycel_drive,blur_pref,sharpen_pref,color_shift_r,color_shift_g,color_shift_b,contrast_pulse,style\n";
    for (int s = 0; s < 4; ++s) {
        for (const auto &e : ctx->dna_species[s].entries) {
            out << "species," << s << "," << e.fitness << ","
                ...
                << e.genome.edge_seek << "," << e.genome.soften_bias << "," << e.genome.mycel_drive << ","
                << e.genome.blur_pref << "," << e.genome.sharpen_pref << ","
                << e.genome.color_shift_r << "," << e.genome.color_shift_g << "," << e.genome.color_shift_b << ","
                << e.genome.contrast_pulse << "," << e.style << "\n";
        }
    }
...
int ms_import_dna_csv(ms_handle_t *h, const char *path) {
    ...
    while (std::getline(in, line)) {
        ...
        std::string edge_str, soften_str, mycel_str, blur_str, sharp_str;
        std::string cr_str, cg_str, cb_str, contrast_str, style_str;
        ...
        std::getline(ss, edge_str, ',');
        std::getline(ss, soften_str, ',');
        std::getline(ss, mycel_str, ',');
        std::getline(ss, blur_str, ',');
        std::getline(ss, sharp_str, ',');
        std::getline(ss, cr_str, ',');
        std::getline(ss, cg_str, ',');
        std::getline(ss, cb_str, ',');
        std::getline(ss, contrast_str, ',');
        std::getline(ss, style_str, ',');
        ...
        if (!edge_str.empty()) g.edge_seek = std::stof(edge_str);
        if (!soften_str.empty()) g.soften_bias = std::stof(soften_str);
        if (!mycel_str.empty()) g.mycel_drive = std::stof(mycel_str);
        if (!blur_str.empty()) g.blur_pref = std::stof(blur_str);
        if (!sharp_str.empty()) g.sharpen_pref = std::stof(sharp_str);
        if (!cr_str.empty()) g.color_shift_r = std::stof(cr_str);
        if (!cg_str.empty()) g.color_shift_g = std::stof(cg_str);
        if (!cb_str.empty()) g.color_shift_b = std::stof(cb_str);
        if (!contrast_str.empty()) g.contrast_pulse = std::stof(contrast_str);
        clamp_genome(g);
        const std::string style = style_str.empty() ? ctx->dna_style : style_str;
        if (pool == "global") {
            ctx->dna_global.add(ctx->params, g, fitness, ctx->evo, ctx->params.dna_global_capacity, style);
        } else {
            if (species >= 0 && species < 4) {
                ctx->dna_species[species].add(ctx->params, g, fitness, ctx->evo, ctx->params.dna_capacity, style);
            }
        }
    }
    return 1;
...
int ms_get_style_dna(ms_handle_t *h, int species, ms_style_dna_t *out) {
    if (!h || !out) return 0;
    auto *ctx = reinterpret_cast<MicroSwarmContext *>(h);
    const DNAMemory *pool = nullptr;
    if (species < 0) {
        pool = &ctx->dna_global;
    } else if (species >= 0 && species < 4) {
        pool = &ctx->dna_species[species];
    } else {
        return 0;
    }
    Genome g{};
    if (!pool || pool->entries.empty()) {
        out->edge_seek = g.edge_seek;
        out->soften_bias = g.soften_bias;
        out->mycel_drive = g.mycel_drive;
        out->blur_pref = g.blur_pref;
        out->sharpen_pref = g.sharpen_pref;
        out->color_shift_r = g.color_shift_r;
        out->color_shift_g = g.color_shift_g;
        out->color_shift_b = g.color_shift_b;
        out->contrast_pulse = g.contrast_pulse;
        out->avg_fitness = 0.0f;
        out->entry_count = 0;
        return 1;
    }
    float total_w = 0.0f;
    ...
    for (const auto &e : pool->entries) {
        float w = e.fitness + 0.01f;
        total_w += w;
        fitness_sum += e.fitness;
    }
    if (total_w <= 0.0f) total_w = 1.0f;
    float inv = 1.0f / total_w;
    float edge = 0.0f, soften = 0.0f, mycel = 0.0f, blur = 0.0f, sharp = 0.0f;
    float cr = 0.0f, cg = 0.0f, cb = 0.0f, cp = 0.0f;
    for (const auto &e : pool->entries) {
        float w = (e.fitness + 0.01f) * inv;
        edge += e.genome.edge_seek * w;
        soften += e.genome.soften_bias * w;
        mycel += e.genome.mycel_drive * w;
        blur += e.genome.blur_pref * w;
        sharp += e.genome.sharpen_pref * w;
        cr += e.genome.color_shift_r * w;
        cg += e.genome.color_shift_g * w;
        cb += e.genome.color_shift_b * w;
        cp += e.genome.contrast_pulse * w;
    }
    out->edge_seek = edge;
    out->soften_bias = soften;
    out->mycel_drive = mycel;
    out->blur_pref = blur;
    out->sharpen_pref = sharp;
    out->color_shift_r = cr;
    out->color_shift_g = cg;
    out->color_shift_b = cb;
    out->contrast_pulse = cp;
    out->avg_fitness = fitness_sum / static_cast<float>(pool->entries.size());
    out->entry_count = static_cast<int>(pool->entries.size());
    return 1;
}
